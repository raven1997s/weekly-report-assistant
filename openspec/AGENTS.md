...
### 需求措辞
- 对于规范性要求使用 SHALL/MUST（除非有意非规范性，否则避免 should/may）
...
- ADDED：引入一个新的功能或子功能，可以作为一个独立的需求。优先使用 ADDED，当变更是正交的（例如，添加"斜杠命令配置"）而不是改变现有需求的语义。
- MODIFIED：改变现有需求的行为、范围或验收标准。始终粘贴完整的、更新的需求内容（标题 + 所有场景）。归档器将用您在此处提供的内容替换整个需求；部分增量将丢失以前的详细信息。
- RENAMED：仅当名称变更时使用。如果您还更改行为，请使用 RENAMED（名称）加上 MODIFIED（内容）引用新名称。
