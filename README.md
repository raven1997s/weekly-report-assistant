...
- 📝 **工作记录**：碎片化记录日常工作，自动识别项目和类型并选择工作状态
- 🏷️ **智能分类**：基于关键词匹配自动归类工作内容
- 🔄 **计划转换**：智能转换上周计划为本周工作记录
...
- 📥 **数据导入**：从 JSON 文件导入数据
- 🔎 **组合筛选**：按项目、工作类型和状态筛选本周记录
- ⚙️ **灵活配置**：自定义项目、类型、工作状态和钉钉配置
...
3. **历史周报** - 查看和管理历史周报归档
4. **回收站** - 查看和恢复已删除的数据
5. **设置** - 配置项目、类型、工作状态、主题和钉钉机器人

### 基本流程

1. **启动应用**：运行 `npm start`
2. **打开浏览器**：访问 http://localhost:5173
3. **记录工作**：在首页输入工作内容，系统自动识别项目和类型，并可选择工作状态
...
4. **全节假日周规则**：如果自然周全是节假日 → 显示"本周无工作日"

**典型场景示例**：

- **元旦补班周**（1/5-1/11）：上周日1/4是补班，工作周从1/4开始，到1/9结束（6个工作日）
- **春节假期周**（2/16-2/22）：全周都是节假日，显示"本周无工作日"
- **国庆前补班周**（9/21-9/27）：上周日9/20是补班，工作周从9/20开始到9/24结束（5个工作日）

**为什么工作周从上周日开始**：

当上周日是补班日时（如1/4、9/20），它会与本周工作日连接成完整的工作周。这样设计的好处：
- ✅ 补班日与工作日连在一起，形成完整的工作周
- ✅ 避免补班日孤立显示，更符合实际工作安排
- ✅ 工作周不会跨越超过周日，保持合理的周范围
...
**records（工作记录表）**：
- id, content, project, workType, status
- createdAt, updatedAt
...
如需排查数据，可在容器外使用 SQLite 命令行读取持久化文件：
...
### Q: 节假日日期不准确？
**A**:
1. 需要更新 `src/utils/date.js` 和 `server/utils/date.js` 中的 `CHINESE_HOLIDAYS` 对象
2. 数据来源：国务院办公厅发布的法定节假日安排
3. 前后端必须同步更新，确保计算逻辑一致
4. 更新后需测试所有节假日场景，确保工作周计算正确

### Q: 为什么工作周从上周日开始？
**A**:
当上周日是补班日时（如1/4、9/20），系统会将它与本周工作日连接成完整的工作周。这样设计的好处：
- ✅ 补班日与工作日连在一起，形成完整的工作周
- ✅ 避免补班日孤立显示，更符合实际工作安排
- ✅ 工作周不会跨越超过周日，保持合理的周范围
